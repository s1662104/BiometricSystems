...
def verificationFAR():
    ...
    for i in range(len(histogram_pn_data)):
        ...
        for t in galley_users:
            ...
            # se il valore è maggiore della soglia adattive di un utente, allora si incrementa il False Accept
...
def topMatch(identity, gallery_target, histogram_gallery_data, hist):
    # inizializzazione delle variabili che contengono il valore più alto di similarity e l'indice in cui si trova
    # identity in gallery_target
    max = 0
...
# mostra il False Rejection Rate e il False Acceptance Rate nell'operazione di riconoscimento del delegato con
# TH adattivo
def evaluationDelegateAsMultiVer():
    # caricamento dei vari dataset
    gallery_target = np.load("npy_db/gallery_target.npy")
    gallery_thresholds = np.load("npy_db/gallery_thresholds.npy")
    histogram_gallery_data = np.load("npy_db/histogram_gallery_data.npy")
    ...
    results1 = delegatesMatch(histogram_pg_data, pg_target, gallery_target, cf_list, users, gallery_thresholds,
                              histogram_gallery_data)
    results2 = delegatesMatch(histogram_pn_data, pn_target, gallery_target, cf_list, users, gallery_thresholds,
                              histogram_gallery_data)
...
# mostra i False Rejection Rate, i False Acceptance Rate, l'Equal Error Rate e la ROC curve nel caso in cui
# si adottano i threshold fissi
def evaluationFixedVerification():
    # caricamento dei vari dataset
    pg_target = np.load("npy_db/pg_target.npy")
    pn_target = np.load("npy_db/pn_target.npy")
    gallery_target = np.load("npy_db/gallery_target.npy")
...
# abbiamo voluto vedere cosa sarebbe successo se avessimo diminuito i threshold adattivi di ogni utente di una piccola
# quantita' per aumentare FRR
def verificationAdaptive():
    ...
    # ottengo il numero totale di utenti genuini e utenti impostori
    tg = len(pg_target)
    ...
    for decrement in np.arange(0.00, 0.11, 0.01):
        ...
        for x in range(similarity_matrix.shape[0]):
            ...
            for y in range(similarity_matrix.shape[1]):
                ...
                similarity = similarity_matrix[x][y]
                # paragoniamo il probe con i template dell'utente
